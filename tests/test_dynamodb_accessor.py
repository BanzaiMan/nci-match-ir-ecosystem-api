...
# from accessors.single_key_accessor import SingleKeyAccessor
# import __builtin__
# import config
#
#
...
#         @patch('accessors.dynamodb_accessor.boto3')
#         # @patch('accessors.ion_reporter_accessor.IonReporterAccessor')
#         def test_get_item(self, ir_id, projection, mock_built, mock_IR_accessor):
#             from ddbmock import connect_boto_patch
#             from ddbmock.database.db import dynamodb
#
#             # Example test
...
#             }
#             __builtin__.environment = 'development'
#             with open(os.path.abspath("config/environment.yml"), 'r') as yaml_file:
#                 __builtin__.environment_config = yaml.load(yaml_file)
#             builtE = mock_built.return_value
#             builtE.environment.return_value = 'development'
#
#
#             instance = mock_IR_accessor.return_value
#             instance.client.return_value = connect_boto_patch
#             instance.resource.return_value = True
#             ion_reporter_accessor = IonReporterAccessor()
#             IonReporterAccessor().table.scan = self.db.layer1.get_item(TABLE_NAME, key)
#             return_value = IonReporterAccessor().scan
#
