...
class TestQueryHelper(unittest.TestCase):
    ...
    @data(
        ({'ion_reporter_id': 'IR_AIO78'}, "'set ion_reporter_id =:ion_reporter_id', {':ion_reporter_id': 'IR_AIO78'}"),
        ({'ion_reporter_id': None}, "'set ion_reporter_id =:ion_reporter_id', {':ion_reporter_id': None}"),
        ({None: 'IR_AIO78'}, "'set None =:None', {':None': 'IR_AIO78'}"),
        ({None: None}, "'set None =:None', {':None': None}"),
    ...
    def test_create_update_expression(self, multidict_query, return_result):
        try:
            ...
            # print "returnresult="+str(return_result)
            assert return_result in str(return_value)
        except Exception, e:
            # print str(e)
            assert str(e) == return_result
